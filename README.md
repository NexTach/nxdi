...
백엔드는 가격 스냅샷, 전일 마감, 배당 동기화를 내부 스케줄러에서 실행합니다. 중복 실행은 MySQL named lock으로 막습니다.
...
- 최초 전환에서는 기존 9개 테이블을 백업한 후 `tb_` 접두사의 snake_case 이름으로 원자적으로 변경합니다. 이후 실행은 멱등하게 건너뜁니다.
